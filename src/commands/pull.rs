...
use crate::storage::image::{load_config, load_manifest, save_layer, store_config, store_manifest};
use clap::ArgMatches;
...
impl PullRequest {
    ...
    pub async fn docker_hub_auth(&mut self) -> PullRequest {
        ...
        let url = format!(
            ...
            self.scope, self.image
        );
        let resp = reqwest::get(url).await;
        match resp {
            Ok(resp) => match resp.json::<DockerToken>().await {
                Ok(data) => {
    ...
    pub async fn docker_hub_manifest(&self) -> Result<ImageManifest, Box<dyn std::error::Error>> {
        ...
        let url = format!(
            ...
            self.scope, self.image, self.tag
        );
    ...
    pub async fn fetch_layers(
        ...
        let url = format!(
            ...
            self.scope, self.image, config_digest
        );
        let client = reqwest::Client::new();
        let resp = client
            .get(url)
...
pub async fn pull(arg: &ArgMatches) -> bool {
    ...
    let mut pull_request = match sscanf::scanf!(image, "{}/{}:{}", String, String, String) {
        Some((scope, image, tag)) => PullRequest::new(scope, image, tag),
        None => {
    ...
    pull_request.fetch_layers(manifest).await.is_ok()
}
